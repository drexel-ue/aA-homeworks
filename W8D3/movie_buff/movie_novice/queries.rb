...
def below_average_years
  ...
  Movie.select(:yr, 'count(title) as bad_movies').where('score < 5').group(:yr).order('yr desc')
end
...
def uma_movies
  ...
  Movie.select('movies.id', 'movies.title', 'movies.yr').joins(:actors).where("actors.name = 'Uma Thurman'").order(:yr)
end
